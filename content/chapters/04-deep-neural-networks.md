...
DNN 像反复折纸。上一层对输入空间造成的变形会传给下一层，下一层不需要重新处理原始空间，而是在已经折叠好的 representation 上继续做新的 ReLU clipping。这样一次新的操作，映射回原始空间后可能同时影响很多区域，这样后一层一次操作，映射回原始输入空间后可能同时影响多个区域，所以深层网络能用相对少的参数产生更多 linear regions。而这里它们本质都是在模拟复杂线段。
